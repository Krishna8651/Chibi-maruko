...
// ---------- CERITA MARUKO-CHAN (FULL BAHASA INDONESIA) ----------
const ceritaMaruko = [
    { 
...
scene.background = new THREE.Color(0xFFE4E1); // Pink pastel
...
function createTeman() {
    ...
    pipiKiri.position.set(-0.16, 0.9, 0.22);
    pipiKiri.scale.set(0.8, 0.4, 0.1);
    group.add(pipiKiri);
    ...
    tanganKiri.position.set(-0.3, 0.6, 0);
    tanganKiri.rotation.z = 0.3;
    tanganKiri.castShadow = true;
    group.add(tanganKiri);
...
function updateDialog() {
    ...
    // Subtitle dalam Bahasa Indonesia (bukan Inggris)
    if (cerita.karakter === "Maruko-chan") {
        subEl.textContent = "(Maruko-chan sedang berbicara)";
    } else if (cerita.karakter === "Teman Maruko") {
        subEl.textContent = "(Teman Maruko sedang berbicara)";
    } else {
        subEl.textContent = "(Momen yang mengharukan)";
    }
    ...
    const happinessValue = Math.min(100, 70 + (indexCeritaSekarang * 2));
    happinessMeter.textContent = happinessValue + '%';
    
    // Efek spesial untuk akhir cerita
    if (cerita.isFinal) {
        for (let i = 0; i < 10; i++) {
    ...
    // Animasi karakter
    if (cerita.emosi === "sangat bahagia" || cerita.emosi === "spesial") {
        maruko.position.y = 0.15;
        teman.position.y = 0.15;
        setTimeout(() => { 
            maruko.position.y = 0; 
            teman.position.y = 0;
        }, 300);
...
function animate() {
    ...
    // Animasi tangan Maruko
    maruko.children.forEach(child => {
        if (child.position.x === -0.35 && child.geometry && child.geometry.type === 'CylinderGeometry') {
...
console.log('🌸 Selamat datang di dunia 3D Chibi Maruko-chan! 🌸');
console.log('🌸 Tekan SPASI untuk lanjut cerita');
